...
public enum PlayerState // 열거형(플레이어의 상태를 나타냄)
{
    Idle,
    Walk,
    Run,
...
public class PlayerController : MonoBehaviour
{
    ...
    {
        Quaternion r = Quaternion.LookRotation(lookDirection); // LookRotaition 바라보는 방향을 저장
        if (rightNow)
        {
    ...
    void AnimationUpdate()
    {
        switch (playerState) 
        {
            case PlayerState.Idle:
                anim.CrossFade(idleAni.name, 0.2f); // 애니메이션 이름으로 처리해도됨
                break;
